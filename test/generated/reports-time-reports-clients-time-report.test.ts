...
test("hrvst reports time-reports clients-time-report", async () => {
  const { stdout } = await execaCommand(
    "node dist/cli.js reports time-reports clients-time-report --help",
  );
  expect(stdout).toMatchInlineSnapshot(`
    "cli.js reports time-reports clients-time-report

    Clients Time Report shows the hours and billable information for each client
